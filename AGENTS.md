...
仕様・バランス・キャラクター性に関する判断を、コードを書きながら独断で下さないでください。判断が必要な箇所に行き当たったら、実装を止めて確認を求めてください。設計文書は `docs/` にあります（企画書・プロトタイプ仕様書）。実装前に該当箇所を参照し、文書と矛盾する実装をしないでください。文書に書かれていないことを勝手に決めない、が原則です。
